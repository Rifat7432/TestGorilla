...
const Option = ({option,correctAnswer,Incorrect,Correct,setIsCorrectAnswer}) => {
    ...
    const isCorrect = () => {
        if(option === correctAnswer){
           ...
           toast.success('Correct Answer',{autoclose:1000 })
            ...
            toast.error(`Incorrect Answer Correct Answer is : ${correctAnswer}`,{autoclose:1000 })
            
        }
    }
    return (
        <button onClick={isCorrect} >
            <div  className={`p-5 border-slate-600  text-slate-700 border-4 md:h-28 md:w-3/4 rounded-2xl  mx-auto hover:shadow-lg ${answer === true ? 'correct' : ''} ${answer === false ? 'in-correct' : ''}`}>
